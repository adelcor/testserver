...
//    std::cout << "Valor de boundary: " << boundary << std::endl;
    ...
    std::string response;
    std::string fileHeader = getRequestedFilename(data);

    if (fileHeader != "static.html" && fileHeader != " /" && !fileHeader.empty()) {
        std::string fileContent = loadStaticContent(fileHeader);
