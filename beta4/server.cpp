...
Server::Server(int port) : port(port), serverSocket(0), keyValuePairs()
{
}
...
//	std::cout << data << std::endl;
	...
	parseRequest(data, this->keyValuePairs);
	printValueForKey("Content-Type", this->keyValuePairs);
	extractFilename("Content-Disposition", this->keyValuePairs);
	printMap(this->keyValuePairs);
