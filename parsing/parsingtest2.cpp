...
int main() {
    ...
    // Establecer socket no bloqueante para el socket del servidor
    ...
    while (true) {
        ...
        for (auto it = clientSockets.begin(); it != clientSockets.end();) {
            if (FD_ISSET(*it, &readSet)) {
                ...
                ssize_t bytesRead;
                do {
                    bytesRead = recv(*it, buffer + totalBytesRead, BUFFER_SIZE - totalBytesRead, MSG_DONTWAIT);
                    if (bytesRead > 0) {
                ...
                } while (bytesRead > 0 && totalBytesRead < BUFFER_SIZE);

                if (totalBytesRead > 0) {
                    // Imprimir la petición recibida
                    std::cout << "Peticion recibida del socket " << *it << ":" << std::endl;
                    std::cout.write(buffer, totalBytesRead);
                    std::cout << std::endl;
