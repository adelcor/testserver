...
void Server::start()
{
    std::vector<int> sockets(5); // Vector para almacenar los descriptores de los sockets
    std::vector<int> ports;

	ports.push_back(5000);
    ports.push_back(5001);
    ports.push_back(5002);
    ports.push_back(5003);
    ports.push_back(5004);

    // Crear y enlazar los sockets en los puertos correspondientes
    for (int i = 0; i < 5; i++) {
        sockets[i] = createServerSocket();
        bindServerSocket(sockets[i], ports[i]);

        if (listen(sockets[i], 1) < 0)
        {
            std::cerr << "Error al escuchar en el socket " << i << std::endl;
            close(sockets[i]);
            exit(1);
        }

        std::cout << "Socket " << i << " a la escucha en el puerto " << ports[i] << "..." << std::endl;
    }

    int maxFd = *std::max_element(sockets.begin(), sockets.end()) + 1; // Obtener el descriptor máximo
    ...
    while (true)
    {
        ...
        // Establecer los sockets en el conjunto de descriptores a observar
        for (int i = 0; i < 5; i++) {
        ...
        // Comprobar cada socket si está listo para aceptar una conexión
        for (int i = 0; i < 5; i++) {
            if (FD_ISSET(sockets[i], &readFds))
            {
                int clientSocket = acceptClientConnection(sockets[i]);
