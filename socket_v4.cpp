        ...
        for (auto it = clientSockets.begin(); it != clientSockets.end(); ) {
            ...
            if (FD_ISSET(clientSocket, &readSockets)) {
                ...
                if (request.find("Content-Disposition: form-data;") != std::string::npos) {
                    ...
                    std::size_t filenamePos = request.find("filename=\"");
                    if (filenamePos != std::string::npos) {
                        filenamePos += 10;  // Avanzar hasta el inicio del nombre del archivo
                        std::size_t filenameEndPos = request.find("\"", filenamePos);
                        if (filenameEndPos != std::string::npos) {
                            std::string filename = request.substr(filenamePos, filenameEndPos - filenamePos);
                            std::cout << "Nombre del archivo: " << filename << std::endl;

                            // Leer los datos del archivo desde el socket
                            std::ofstream outputFile(filename, std::ios::binary);
                            if (outputFile) {
                                // Continuar leyendo del socket hasta que se reciba todo el archivo
                                while (true) {
                                    ...
                                    if (bytesRead == 0) {
                                        // El cliente cerró la conexión antes de terminar de enviar el archivo
                                        std::cerr << "El cliente cerró la conexión antes de enviar el archivo completo." << std::endl;
                                        close(clientSocket);
                                        it = clientSockets.erase(it);
                                        break;
                                    }
                                ...
                                std::cout << "Archivo recibido y guardado exitosamente." << std::endl;
                            } else {
