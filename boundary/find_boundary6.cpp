...
int main() {
    ...
    if (fileStartPos != std::string::npos && fileEndPos != std::string::npos) {
        ...
        std::size_t fileContentStartPos = data.find(searchStr, fileStartPos);
        if (fileContentStartPos != std::string::npos) {
            fileContentStartPos += searchStr.length();
            fileContent = data.substr(fileContentStartPos, fileEndPos - fileContentStartPos);
