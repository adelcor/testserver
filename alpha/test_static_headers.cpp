...
std::string getRequestMethod(const std::string& requestData) {
    std::size_t startPos = requestData.find(" ") + 1;
    std::size_t endPos = requestData.find(" ", startPos);
    if (endPos != std::string::npos) {
        return requestData.substr(startPos, endPos - startPos);
    }
    return "";
}
...
std::string loadStatic() {
    std::ifstream inputFile("static.html", std::ios::binary);
    if (!inputFile) {
        std::cerr << "Error al abrir el archivo: static.html" << std::endl;
        return "";
    }
    std::string content((std::istreambuf_iterator<char>(inputFile)), std::istreambuf_iterator<char>());
    inputFile.close();
...
void handleClientRequest(int clientSocket) {
    ...
    std::cout << "Método de solicitud: " << requestMethod << std::endl;
	...
	std::string staticContent = loadStatic();
	sendResponse(clientSocket, staticContent);
    } else {
        std::cerr << "Método de solicitud no válido." << std::endl;
    }
