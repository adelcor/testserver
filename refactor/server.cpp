...
void Server::start() 
{
	...
	while (true) 
    	{
		int clientSocket;
		int result = select(serverSocket + 1, &readfds, &writefds, nullptr, nullptr);
...
//	printValueForKey("Transfer-Encoding", keyValuePairs);	
	...
	if(FD_ISSET(clientSocket, writefds))
	{
		...
		{
			handleGetRequest(clientSocket, this->data);
		}
		
		else if(requestMethod == "POST")
		{
			handlePostRequest(clientSocket, this->data, keyValuePairs);
		}
		else if(requestMethod == "DELETE")
		{
			handleDeleteRequest(clientSocket, this->data);
		}
		else
		{
